...
Linux::Linux(const Driver &D, const llvm::Triple &Triple, const ArgList &Args)
  ...
  if (!IsMips && !IsHexagon) {
    if (Distro.IsOpenSUSE() || Distro == Distro::UbuntuLucid ||
        ...
        (IsAndroid && llvm::TripleUtils::isAndroidVersionLT(Triple, 23)))
...
std::string Linux::getDynamicLinker(const ArgList &Args) const {
  ...
  if (Triple.isAndroid()) {
    if (getSanitizerArgs(Args).needsHwasanRt() &&
        !llvm::TripleUtils::isAndroidVersionLT(Triple, 34) && llvm::TripleUtils::isArch64Bit(Triple)) {
    ...
    return llvm::TripleUtils::isArch64Bit(Triple) ? "/system/bin/linker64" : "/system/bin/linker";
  }
  if (Triple.isMusl()) {
    std::string ArchName;
    bool IsArm = false;
